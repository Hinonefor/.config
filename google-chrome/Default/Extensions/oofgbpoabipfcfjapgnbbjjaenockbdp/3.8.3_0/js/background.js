...
var Utils = function () {
	...
	_createClass(Utils, [{
		...
		value: function isTimeoutSettingValid(timeoutSettings) {
			if (timeoutSettings && this.isInt(timeoutSettings) && timeoutSettings >= 10000 && timeoutSettings < 40000) {
				return true;
			}
...
var Api = function () {
    ...
    */
      ...
      */
					...
					_this.getOS().then(function (os) {
						...
						_this.request(_endpoints2.default.TIER, 'POST', parameter, function (response) {
							...
							if (response && response != "" && response.retcode === 200 && response.data && response.data.hasOwnProperty("mainbase") && Array.isArray(response.data.mainbase) && response.data.mainbase.length > 2 && response.data.hasOwnProperty("tierbase") && Array.isArray(response.data.tierbase) && response.data.tierbase.length > 2) {
								...
								chrome.storage.local.set((_chrome$storage$local6 = {}, _defineProperty(_chrome$storage$local6, _ServiceMeta2.default.STORAGEKEYS.SERVERSJSON, response.data), _defineProperty(_chrome$storage$local6, _ServiceMeta2.default.STORAGEKEYS.TIERNEXTUPDATE, nextUpdate), _chrome$storage$local6));
					...
					_this.getOS().then(function (os) {
						...
						_this.request(_endpoints2.default.CLIENTUPDATES, 'POST', parameter, function (response) {
							...
							if (response && response != "" && response.retcode === 200 && response.data && response.data.hasOwnProperty("action") && (response.data.action === "update" || response.data.action === "stop") && response.data.hasOwnProperty("curversion") && response.data.hasOwnProperty("minversion") && response.data.hasOwnProperty("sources") && Array.isArray(response.data.sources) && response.data.sources.length > 0) {
								...
								chrome.storage.local.set((_chrome$storage$local9 = {}, _defineProperty(_chrome$storage$local9, _ServiceMeta2.default.STORAGEKEYS.VERSIONSTATUS, response.data), _defineProperty(_chrome$storage$local9, _ServiceMeta2.default.STORAGEKEYS.VERSIONSTATUSNEXTUPDATE, nextUpdate), _chrome$storage$local9));
							} else {
		...
		key: 'registerCID',
		value: function registerCID(callback) {
			var _this = this;

			chrome.storage.local.get([_ServiceMeta2.default.STORAGEKEYS.BASELINK, _ServiceMeta2.default.STORAGEKEYS.CID], function (storage) {
				var baselink = storage[_ServiceMeta2.default.STORAGEKEYS.BASELINK];
				if (!baselink) {
					if (callback) callback(false);
					return;
				}
				var cid = storage[_ServiceMeta2.default.STORAGEKEYS.CID];
				...
				_this.getOS().then(function (os) {
					...
					_this.request(_endpoints2.default.REGISTERCID, 'POST', parameter, function (response) {
						if (response && response != "" && response.retcode === 200 && response.hasOwnProperty("cid")) {
							chrome.storage.local.set(_defineProperty({}, _ServiceMeta2.default.STORAGEKEYS.CID, response.cid), function () {
								if (callback) callback(response.cid);
							});
						} else {
							if (callback) callback(false);
						}
		...
		value: function request(endpoint, method, parameter, callback, ignoreFail) {
			...
			chrome.storage.local.get([_ServiceMeta2.default.STORAGEKEYS.BASELINK, _ServiceMeta2.default.STORAGEKEYS.BASELINKTYPE, _ServiceMeta2.default.STORAGEKEYS.TIMEOUTSETTINGS, _ServiceMeta2.default.STORAGEKEYS.CID], function (storage) {
				var cid = storage[_ServiceMeta2.default.STORAGEKEYS.CID];
				...
				if (method == "POST" && parameter && cid) parameter.cid = cid;
...
var _RegisterCID = __webpack_require__(51);

var _RegisterCID2 = _interopRequireDefault(_RegisterCID);
...
var RegisterCID = function () {
  function RegisterCID() {
    _classCallCheck(this, RegisterCID);
  ...
  _createClass(RegisterCID, [{
    key: 'registerCID',
    value: function registerCID() {
      _Api2.default.registerCID();
    }
  }]);
...
exports.default = new RegisterCID();
